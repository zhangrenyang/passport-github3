...
function Strategy(options, verify) {
  ...
  const agent = options.agent || process.env.HTTPS_PROXY || process.env.https_proxy || process.env.HTTP_PROXY || process.env.http_proxy;
  if (agent) {
    this._oauth2.setAgent(new HttpsProxyAgent(agent));
  }
}
